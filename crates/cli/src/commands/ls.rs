...
async fn execute_incomplete(args: &LsArgs, formatter: &Formatter) -> ExitCode {
    ...
    let (alias_name, bucket, prefix) = match parse_incomplete_ls_path(&args.path) {
        Ok(path) => path,
        Err((code, message)) => {
...
async fn list_incomplete_uploads(
    ...
    let uploads = match collect_multipart_uploads(options, |page_options| {
...
fn parse_incomplete_ls_path(path: &str) -> Result<(String, String, String), (ExitCode, String)> {
    ...
    if parts.len() < 2 || parts[0].is_empty() || parts[1].is_empty() {
        return Err((
            ExitCode::UsageError,
            "--incomplete requires a bucket path in the form alias/bucket[/prefix]".to_string(),
        ));
    }
...
mod tests {
    ...
    #[test]
    fn incomplete_path_preserves_a_nested_prefix() {
        let (alias, bucket, key) = parse_incomplete_ls_path("local/archive/2026/backup.tar")
            .expect("nested prefix should parse");
    ...
    #[test]
    fn incomplete_path_accepts_bucket_and_prefix_modes() {
        assert_eq!(
            parse_incomplete_ls_path("local/archive")
                .expect("bucket path should parse")
                .2,
            ""
        );
        assert_eq!(
            parse_incomplete_ls_path("local/archive/logs/")
                .expect("prefix path should parse")
                .2,
            "logs/"
        );
    }
