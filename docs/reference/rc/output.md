...
| [`output_v3.json`](../../../schemas/output_v3.json) | New capability, version, lock, multipart, watch, usage, metrics, scanner-status, storage-info, KMS, and admin-operation families | Contract for new implementations |
...
- `data` for successful records, or `error` for failed records;
...
8. For KMS operations, treat `not-configured` as a successful service state, dispatch lifecycle results by `key_create`, `key_delete`, or `key_cancel_deletion`, and never expect secret configuration or data-key fields.
